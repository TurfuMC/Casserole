...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(message.channel, "❌ | **Pas assez de chansons dans la file d'attente pour mélanger !**");
        player.queue.shuffle();
        await client.sendTime(message.channel, "✅ | Mélange la file d'attente !");
    },
    SlashCommand: {
        /**
     *
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!player) return client.sendTime(interaction.channel, "❌ | **Rien ne joue en ce moment...**");
            if (!player.queue || !player.queue.length || player.queue.length === 0) return client.sendTime(interaction, "❌ | **Pas assez de chansons dans la file d'attente pour mélanger !**");
            player.queue.shuffle();
            client.sendTime(interaction, "✅ | Mélange la file d'attente !");
        },
    },
};
