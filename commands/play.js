...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!SearchString) return client.sendTime(message.channel, `**Usage - **\`${GuildDB.prefix}play [musique]\``);
        let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
        let Searching = await message.channel.send(":mag_right: Je cherche la musique j'ai peur de se que vais trouver...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,"❌ | **Nœud Lavallink non connecté c'est la merde**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.addField("En file d'attente", `\`${Searched.tracks.length}\` musiques`, false);
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Aucune correspondance trouvée pour - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Playlist ajoutée à la file d'attente`, client.botconfig.IconURL);
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.addField("En file d'attente", `\`${Searched.tracks.length}\` musique`, false);
                ...
                switch (res.loadType) {
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        SongAdded.addField("En file d'attente", `\`${res.tracks.length}\` musiques`, false);
                    ...
                    case "SEARCH_RESULT":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) {
                            ...
                            SongAddedEmbed.addField("Auther", track.author, true);
